...
def login_view(request):
    if request.method == "POST":
        ...
        return render(request, "invoices/login.html", {"error": "Invalid credentials"})
...
def generate_invoice(request):
    ...
    if request.method == "POST":
        ...
        ROW_HEIGHT = 9.5
                  ...
                  # ──────── INVOICE BREAKUP RECTANGLES (EXACT ORIGINAL ALIGNMENT) ────────
        GROSS_VALUE_RECT     = (535, 260,     590, 280)
        DISCOUNT_VALUE_RECT  = (535, 277.58,  590, 287.63)   # y same rakha (template ka exact position)
        ...
        table_rect = fitz.Rect(50, ROW_START_Y - 2, 580, ROW_START_Y + (len(names) * ROW_HEIGHT) + 5)
        ...
        # ✅ FINAL DISCOUNT FIX
        write_in_rect_right(page, DISCOUNT_VALUE_RECT,  f"-{abs(total_discount):.2f}", 9)   # ← space hataya
        write_in_rect_right(page, NET_PAYABLE_RECT,     f"{total_net:.2f}", 9)
        write_in_rect_right(page, COMPANY_TOTAL_RECT,   f"{total_net:.2f}", 9)

        output_file = os.path.join(OUTPUT_DIR, f"{invoice.invoice_no}.pdf")
